...
"""Compute OBS vs CMIP6 AMIP feedback fits over 2003–2014 with manifest lookup.
 ...
 python -m scripts.feedback_fit_cmip_vs_obs   --metric lambda_sw   --regions SEP,SEA,NEP,NEA,SEI   --cmip-csv output/cmip_amip_lambda_sw_2003-2014_models.csv

 eis
 # y：低云量（CMIP6 里是 CLLISCCP）
python -m scripts.make_cmip_panel_models --var cllisccp --start 2003 --end 2014 \
  --out output/tables/cmip_panel_cllisccp_2003-2014_models.csv

python -m scripts.build_cmip_monthly_stack \
  --y output/tables/cmip_panel_cllisccp_2003-2014_models.csv \
  --x output/tables/cmip_panel_eislts_2003-2014_models.csv \
  --y-name lcf --x-name eislts \
  --out output/cmip_amip_dlcf_deis_2003-2014_models.csv

python -m scripts.feedback_fit_cmip_vs_obs \
  --metric dLCF_dEIS \
  --regions SEP,SEA,NEP,NEA,SEI \
  --cmip-csv output/cmip_amip_dlcf_deis_2003-2014_models.csv \
  --fig-dir fig/models
...
def gather_cmip_candidates(
    ...
    candidates.append(Path(DEFAULT_OUTPUT_DIR) / "cmip_amip_monthly_2003-2014.csv")
    for var in metric_cfg.cmip_y_vars:
        candidates.append(
            Path(DEFAULT_OUTPUT_DIR) / f"cmip_amip_{var}_vs_obs_2003-2014.csv"
        )
...
def make_scatter(
    metric_cfg, regions, obs_results, model_results, plot_infos, fig_path, logger,
    label_topn=3,  # 每区仅标注离 1:1 线偏差最大的 N 个模型
):
    import matplotlib.pyplot as plt
    import numpy as np

    # 画布布局：每区一格（5 区 → 1x5 或 2x3）
    n = len(regions)
    ...
    all_vals = []
    for r in regions:
        all_vals.append(obs_results[r]["b"])
        for row in model_results.get(r, []):
    ...
    for ax, region in zip(axes, regions):
        obs_b  = obs_results[region]["b"]
        obs_se = obs_results[region]["SE_HAC"]

        # 1) OBS: 阴影 95% CI + 中线
        ax.axvspan(obs_b - 1.96*obs_se, obs_b + 1.96*obs_se, color="0.85", alpha=0.6, zorder=0)
        ax.axvline(obs_b, color="0.3", ls="--", lw=1.2, label="OBS ±95% CI")

        # 2) 模型：水平误差棒（x 方向），y 用类别索引避免重叠
        models = model_results.get(region, [])
        if not models:
            ax.set_title(region); ax.set_xlim(xlim); ax.grid(ls=":", alpha=0.4); continue

        names  = [m["model"] for m in models]
        xs     = np.array([m["b"] for m in models], dtype=float)
        xerr   = np.array([m["SE_HAC"] for m in models], dtype=float)
        yind   = np.arange(len(models))[::-1]  # 上到下
        ax.errorbar(xs, yind, xerr=xerr, fmt="o", ms=4, lw=1.0, capsize=2,
                    mfc="white", mec="k", ecolor="0.2", alpha=0.9)

        # 只标注离 OBS 的 |x-obs| 最大的 Top-N
        idx = np.argsort(np.abs(xs - obs_b))[::-1][:label_topn]
        for i in idx:
            ax.annotate(names[i],
                        xy=(xs[i], yind[i]),
                        xytext=(4, 0),
    ...
    fig.supylabel(f"Models (sorted)", x=0.005)
    fig.supxlabel(f"λ_cld,SW ({metric_cfg.unit_label})")
    fig.suptitle(f"{metric_cfg.title} comparison ({', '.join(regions)}, 2003–2014)", y=0.98)
